def patch_file(file_name):
    ...
    # Find the block we inserted and modify it to strip spaces
    old_code = """        let key = headers[j] ? String(headers[j]).trim() : "Col" + j;
        obj[key] = row[j];
        // add case variants to make it bulletproof
        if (key.toLowerCase() === 'stokisid') { obj['StokisID'] = row[j]; obj['stokisId'] = row[j]; }"""
        
    new_code = """        let key = headers[j] ? String(headers[j]).trim() : "Col" + j;
    ...
    # We need to replace the entire block we added in fix_getdata.py
    import re
    # Match from "let key = headers[j]" up to "if (key.toLowerCase() === 'role')"
